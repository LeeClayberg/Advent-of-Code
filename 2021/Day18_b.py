...
def number_sum(structure):
    if isinstance(structure, list):
        return number_sum(structure[0]) + number_sum(structure[1])
    else:
        return structure


structures = list(map(lambda number: ast.literal_eval(number), numbers))
max_magnitude = 0
for a in range(len(structures)):
    first_num = structures[a]
    rest = structures[:a] + structures[a+1:]
    for b in range(len(rest)):
        second_num = rest[b]
        num = number_magnitude(simplify_number([first_num, second_num]))
        max_magnitude = max(max_magnitude, num)
print(max_magnitude)
