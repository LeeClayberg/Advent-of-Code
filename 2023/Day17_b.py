...
with open("files/day17.txt", "r") as file_stream:
    ...
    turning = {'U': {'L', 'R'}, 'D': {'L', 'R'}, 'L': {'U', 'D'}, 'R': {'U', 'D'}}
                ...
                for i in range(0, 10):
                    for dir_pros in [key for key in directions.keys() if key != opposite[dir_curr]]:
                        ...
                        if 0 > new_pos[0] or new_pos[0] >= len(grid[0]) or 0 > new_pos[1] or new_pos[1] >= len(grid):
                            continue
                        if i < 3 and dir_pros in turning[dir_curr]:
                            continue
                        if i == 9 and dir_pros == dir_curr:
                            continue
    ...
    for node in end_nodes:
        try:
            ...
            d_list = nx.dijkstra_path(G, (0, 0, 'S', 0), node)
            d_list = [(x, y) for x, y, _, _ in d_list]
