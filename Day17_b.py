...
for cycle in range(0, 6):
    ...
    all_w = list(map(lambda x: x[3], positions))

    range_positions = [(x, y, z, w) for w in range(min(all_w)-1, max(all_w)+2) for z in range(min(all_z)-1, max(all_z)+2) for y in range(min(all_y)-1, max(all_y)+2) for x in range(min(all_x)-1, max(all_x)+2)]
    ...
    for position in positions.keys():
        neighbors = [(x, y, z, w) for w in range(position[3]-1, position[3]+2) for z in range(position[2]-1, position[2]+2) for y in range(position[1]-1, position[1]+2) for x in range(position[0]-1, position[0]+2)]
        active = 0
